...
const coordinatesRequests = (locations) => locations.map(async location => {
...
Promise.all([
  ...
  const comment = '// This is auto generated code by running "generateCoordinates.js". Do NOT make changes to this file directly!!\n\n'

  // fs.writeFile('../functions/coordinates/cse.json', comment + JSON.stringify(responses[0]), err => {
  //   if (err) throw err
  //   console.log('Coordinates retrieved!')
  // })

  // // SNAP and TANF have the same offices
  // fs.writeFile('../functions/coordinates/tanf.json', comment + JSON.stringify(responses[1]), err => {
  //   if (err) throw err
  //   console.log('Coordinates retrieved!')
  // })

  // // SNAP and TANF have the same offices
  // fs.writeFile('../functions/coordinates/snap.json', comment + JSON.stringify(responses[1]), err => {
  //   if (err) throw err
  //   console.log('Coordinates retrieved!')
  // })
  ...
  // SNAP and TANF have the same offices
  fs.writeFile('../functions/coordinates/tanf.json', comment + JSON.stringify(responses[0]), err => {
    if (err) throw err
    console.log('Coordinates retrieved!')
  })

  // SNAP and TANF have the same offices
  fs.writeFile('../functions/coordinates/snap.json', comment + JSON.stringify(responses[0]), err => {
    if (err) throw err
    console.log('Coordinates retrieved!')
  })
})
