...
import { clearError, setError } from '../ducks/errorSlice'
import { hideButtonBar, showButtonBar } from '../ducks/buttonBarSlice'
                ...
                if (type === 'payload') {
                    ...
                    if ('disableInput' in payload) {
                        ...
                        if (Object.keys(payload).length > 1) {
                            const modifiedPayload = omit(payload, [
                                'disableInput',
                            ])
                            dispatch({
                                type: RECEIVE_WEBHOOK_DATA,
                                modifiedPayload,
                            })
                        }
...
const sendToDialogflow = (
    type: 'textRequest' | 'eventRequest',
    payload: any
) => {
    return (dispatch, getState) => {
...
export const sendMessageWithDialogflow = message =>
    sendToDialogflow('textRequest', message)
