...
class ChatWindow extends PureComponent<any> {
    ...
    componentDidMount() {
        ;(this.componentRef.current as any).addEventListener(
            'wheel',
            this.handleWheel,
            { passive: false }
        )
            ...
            if (msg.loading) {
                ...
                msg.responses.forEach((res, i) => {
                    if (
                        ...
                        const botMessage = merge(res, {
                            ...metadata,
                            key: `${metadata.key}_${i}`,
                        })
    ...
    createMessageElements = () => {
        ...
        const indexFirstElementInLastResponse = this.findIndexFirstElementInLastResponse(
            sortedNewMessages
        )
        ...
        sortedNewMessages.forEach((msg, i) => {
            ...
            if (msg.loading) {
                ...
                msgElements.push(
                    buildFeedbackResponse(msg, shouldScrollIntoView)
                )
            } else {
                msgElements.push(
                    buildBotTextMessage(
        ...
        this.setState(
            prevState => ({ ...prevState, messageElements: msgElements }),
            () => {
                ...
                if (document.getElementsByClassName('scrollIntoView')[0]) {
                    ...
                    setTimeout(
                        () =>
                            document
                                .getElementsByClassName('scrollIntoView')[0]
