...
const getSuggestionsFromMessage = message =>
    message && message.responses
        ? message.responses.filter(m => m.type === 'suggestion')[0].suggestions
        : []
...
function conversation(state = initialState, action) {
    ...
    switch (action.type) {
        case t.INITIATE_LOADING:
            ...
            suggestions = getSuggestionsFromMessage(
                findLastMessageWithSuggestions(newMessages)
            )
        ...
        case t.SAVE_RESPONSE:
            suggestions = getSuggestionsFromMessage(
                findLastMessageWithSuggestions(action.newConversationArray)
            )
